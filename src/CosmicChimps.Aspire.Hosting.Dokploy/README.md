...
// The dashboard's own compose service name — "<PublishToDokploy name>-compose-dashboard" — which
// is the hostname every sender resolves. It MUST be in AllowedHosts: see "Silent telemetry loss"
// below, where leaving it out drops all telemetry with no error anywhere.
var dashboardHost = "myapp-compose-dashboard";
...
dokploy.WithDokployDashboard(dashboard =>
{
    dashboard
        ...
        // Required together behind a proxy — see below. The ingest host is not optional: without
        // it the dashboard runs and accepts browsers while silently rejecting every sender.
        .WithEnvironment(
            "AllowedHosts",
...
| `AllowedHosts` (set, but missing the ingest host) | Dashboard looks perfect and stays permanently empty. See below — this is the worst of the three |
...
**Setting `AllowedHosts` to fix the browser 400 will break telemetry unless the dashboard's own
service name is in the list.** This is the single most expensive mistake in this document, because
nothing reports it.
...
The fix is to append the ingest host unconditionally — including over any operator override, since
omitting it does not degrade the deployment, it disables telemetry entirely:

```csharp
var dashboardHost = $"{dokployAppName}-compose-dashboard";   // the PublishToDokploy name
dashboard.WithEnvironment("AllowedHosts", $"{domain};localhost;127.0.0.1;{dashboardHost}");
```

Write it as the **same string** you use for `OTEL_EXPORTER_OTLP_ENDPOINT`. The publisher substitutes
service names in a `;`-separated list segment by segment, so both are rewritten to the Dokploy app
name together and cannot drift apart.
